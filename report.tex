...
\author{
Firstname Lastname \email{sahbi.hanafi@student-cs.fr} \\ % student 1
Firstname Lastname \email{wadi.moughanim@mail.com} % student 2
}
...
\begin{solution}
The influence of $R$ on the kernel functions $\hat{g}_m$ is that it controls the width of the support of the kernel functions in the spectral domain. As $R$ increases, the kernels become wider and smoother, which means they will capture a broader range of frequencies. This can affect the localization of the wavelets in the spectral domain. For $R=1$, the kernels are very localized and affect a narrow range of frequencies. As $R$ increases to 3 and 5, the kernels cover a wider range of frequencies and the transitions between adjacent kernels become smoother. This implies that a larger $R$ can better capture the overall structure of the signal at the cost of spectral resolution.
\begin{figure}
    \centering
